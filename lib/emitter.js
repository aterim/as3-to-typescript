...
function transformAST(node, parentNode) {
    ...
    var newNode = new Node(node.kind, node.start, node.end, node.text, [], parentNode, node.isInt, node.isUint);
...
var state;
var output;
var preInitVar = false;
function emit(ast, source, options, flag) {
    if (flag == "-iv") {
        preInitVar = true;
    }
    data = {
...
function catchup(index) {
    ...
    while (state.index !== index) {
        ...
        if (waitEnd.length) {
            if (data.source[state.index] == "\n" || data.source[state.index] == "\r" || data.source[state.index] == ";" || (data.source[state.index] == ")")) {
                insert(")");
                waitEnd.pop();
            }
            // bracketCounter
            if (data.source[state.index] == "(")
                waitEnd[waitEnd.length - 1]++;
            if (data.source[state.index] == ")")
                waitEnd[waitEnd.length - 1]--;
        }
    }
